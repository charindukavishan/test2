...
@Component({
  ...
  templateUrl: './long-vehicle.component.html',
  styleUrls: ['./long-vehicle.component.scss']
})
export class LongVehicleComponent implements OnInit {
    data={
        keeperId:'',
        slotId:''
  ...
  constructor(private flashMessage:FlashMessagesService,private modalService: NgbModal,private service:RegserviceService ,private router: Router) {}
    ...
    ngOnInit() {
      ...
      this.service.getKeeperProfile().subscribe(profile =>{
        ...
        this.amounts=this.user.alocatedSlots5;
        this.num= this.user.alocatedSlots5;
        this.charge=this.user.hourCharge5;
        this.Arr=this.user.type5;
